...
export async function connectToWhatsApp(handleMessage) {
    ...
    sock.sendMessage = async (jid, content, options) => {
        if (content) {
            ...
            if (Array.isArray(content.mentions)) {
                content.mentions = content.mentions.map(cleanJid);
            }
            if (content.contextInfo && Array.isArray(content.contextInfo.mentionedJid)) {
                content.contextInfo.mentionedJid = content.contextInfo.mentionedJid.map(cleanJid);
            }
            ...
            const cleanText = (val) => {
                ...
                return val.replace(/\b(\d{7,16}):(\d+)\b/g, '$1')
                          ...
                          .replace(/(\d+):(\d+)@lid/g, '$1@lid')
                          .replace(/@([a-zA-Z0-9]+)(:\d+)?@(s\.whatsapp\.net|lid)\b/g, '@$1')
    ...
    sock.ev.on('group-participants.update', async (anu) => {
        try {
            ...
            // Fetch group metadata to help map LIDs to phone number JIDs
            let groupMetadata = null;
            try {
                groupMetadata = await sock.groupMetadata(id);
            } catch (_) {}

            const resolveParticipantJid = (numJid) => {
                if (!numJid) return '';
                const cleaned = numJid.split('@')[0].split(':')[0] + '@' + (numJid.includes('@lid') ? 'lid' : 's.whatsapp.net');
                if (cleaned.endsWith('@lid') && groupMetadata && groupMetadata.participants) {
                    const match = groupMetadata.participants.find(p => p.lid && (p.lid.split('@')[0].split(':')[0] + '@lid') === cleaned);
                    if (match && match.id) {
                        return match.id.split('@')[0].split(':')[0] + '@s.whatsapp.net';
                    }
                }
                return cleaned;
            };
            ...
            if (action === 'add' && groupDb.welcome) {
                for (const num of participants) {
                    const cleanJid = resolveParticipantJid(num);
