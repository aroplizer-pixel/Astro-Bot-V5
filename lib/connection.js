...
export async function connectToWhatsApp(handleMessage) {
    ...
    // الترحيب والمغادرة التلقائي للأعضاء الجدد والراحلين
        ...
        try {
            ...
            if (action === 'add' && groupDb.welcome) {
                for (const num of participants) {
                    ...
                    let welcomeMsg = groupDb.welcomeMessage || 'مرحباً بك يا @user في المجموعة! 👋';
                ...
                for (const num of participants) {
                    const userNumber = num.split('@')[0];
                    let goodbyeMsg = groupDb.goodbyeMessage || 'وداعاً يا @user.. 🥺';
