...
export function getMediaFromMessage(msg) {
    if (!msg?.message) return { messageContent: null, type: null };
    let m = msg.message;
    if (m.viewOnceMessage?.message) m = m.viewOnceMessage.message;
    else if (m.viewOnceMessageV2?.message) m = m.viewOnceMessageV2.message;
    else if (m.ephemeralMessage?.message) m = m.ephemeralMessage.message;
    ...
    if (quoted) {
        ...
        if (q.imageMessage) return { messageContent: q.imageMessage, type: 'image' };
        if (q.videoMessage) return { messageContent: q.videoMessage, type: 'video' };
        if (q.audioMessage) return { messageContent: q.audioMessage, type: 'audio' };
        if (q.stickerMessage) return { messageContent: q.stickerMessage, type: 'sticker' };
        if (q.documentMessage) return { messageContent: q.documentMessage, type: 'document' };
    }
...
/**
 * تنظيف الـ JID من رقم الجهاز الفرعي مع الحفاظ على النطاق الأصلي (s.whatsapp.net أو lid)
 */
