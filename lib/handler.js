...
export async function checkSenderAdmin(sock, from, sender) {
    ...
    const cleanSender = sender ? (sender.split('@')[0].split(':')[0] + '@s.whatsapp.net') : '';
    const admins = metadata.participants
        .filter(p => p.admin === 'admin' || p.admin === 'superadmin')
        .map(p => p.id.split('@')[0].split(':')[0] + '@s.whatsapp.net');
    const cleanBot = sock.user.id.split('@')[0].split(':')[0] + '@s.whatsapp.net';
    return {
        isSenderAdmin: admins.includes(cleanSender),
        isBotAdmin: admins.includes(cleanBot),
        admins
    };
}
            ...
            try {
                ...
                if (groupDb.antiDelete) {
                    ...
                    if (originalMsg) {
                        ...
                        const cleanDeletedSender = rawDeletedSender ? (rawDeletedSender.split('@')[0].split(':')[0] + '@s.whatsapp.net') : '';
                        const deletedSenderNumber = cleanDeletedSender.split('@')[0];
                        ...
                        if (mediaType) {
                            ...
                            if (mediaType === 'image') {
                                ...
                                await sock.sendMessage(from, { text: notificationText, mentions: [cleanDeletedSender] });
    ...
    if (isGroup) {
        try {
            ...
            if (groupMetadata) {
                ...
                const isBotAdmin = admins.includes(cleanBot);
                ...
                if (groupDb.antiLink && !isOwner && !isSenderAdmin) {
                    ...
                    if (hasLink) {
                        if (isBotAdmin) {
                            ...
                            await sock.sendMessage(from, {
                                text: `⚠️ تنبيه: @${senderNumber} أرسل رابطاً في المجموعة! يرجى طرده (البوت ليس مشرفاً لحذفه).`,
                                mentions: [sender]
                            });
                        }
                    }
